...
class LiquidacionDePlomoPlata extends Liquidacion{
    static searchable = true

    Integer numeroLiquidacionPlomoPlata //e.g.: GENERADO: 1, PARA MOSTRAR: 0001
    String conjuntoPlomoPlata
    ...
    static constraints = {
        numeroLiquidacionPlomoPlata(nullable: true)
        conjuntoPlomoPlata(blank: true)
    ...
    def beforeInsert = {
        def c = LiquidacionDePlomoPlata.createCriteria()
        def results = c {
            projections {
                max('numeroLiquidacionPlomoPlata')
            }}
        def maxNumeroLiquidacion = results.get(0)?: 0
        this.numeroLiquidacionPlomoPlata = maxNumeroLiquidacion + 1
        this.conjuntoPlomoPlata = "-"
    ...
    def afterInsert = {
        ...
        LiquidacionDePlomoPlata.withNewTransaction {

            def liquidacionDePlomoPlata = LiquidacionDePlomoPlata.get(this.id)
            ...
            retencionesJSON.each {
                ...
                if(!codigo.equals("-")){
                    def liquidacionDePlomoPlataRetenciones = new LiquidacionDePlomoPlataRetenciones(
                            ...
                            monto: monto)
                    liquidacionDePlomoPlataRetenciones.save(failOnError: true)

                    def retencionPorPagarComplejo = new RetencionPorPagarComplejo(
                            liquidacionId: liquidacionDePlomoPlata.id,
                            codigo: codigo,
                            cantidadDescuento: cantidad,
                            unidadDeDescuento: unidad,
                            tipoDeRetencion: tipo,
                            ...
                            asignacionDelDescuento: asignacion,
                            monto: monto,
                            lote: liquidacionDePlomoPlata.recepcionDeComplejo.toString(),
                            kilosNetosSecos: liquidacionDePlomoPlata.kilosNetosSecos,
                            valorOficialNeto: liquidacionDePlomoPlata.valorNetoMineralEnBolivianos,
                            recepcionDeComplejo: liquidacionDePlomoPlata.recepcionDeComplejo,
                            tipoDeMineral: liquidacionDePlomoPlata.recepcionDeComplejo.tipoDeMineral,
                            empresa: liquidacionDePlomoPlata.empresa,
//                            fechaDeRegistro: liquidacionDePlomoPlata.fechaDeLiquidacion,
                            fechaDeRegistro: liquidacionDePlomoPlata.recepcionDeComplejo.fechaDeRecepcion,
                            pagado: "NO"
                    )
    ...
    def afterUpdate = {
        ...
        def retencionesAnteriores = LiquidacionDePlomoPlataRetenciones.findAllByLiquidacionDePlomoPlata(this)
...
//                            fechaDeRegistro: liquidacionDePlomoPlata.fechaDeLiquidacion,
        ...
        */
        ...
        def reimpresion = new Reimpresion(
                ...
                nombreReporte: "RELIQUIDACION DE PLOMO PLATA",
                identificadorDocumento: numeroLiquidacionPlomoPlata,
                lote: lote,
