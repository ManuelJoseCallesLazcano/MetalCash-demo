...
class RecepcionDeOro extends Recepcion {
    Integer loteOro //e.g.: GENERADO: 1, PARA MOSTRAR: SB000001
    Integer codigoDepositoOro = 0
    ...
    def beforeInsert = {
        ...
        def c = RecepcionDeOro.createCriteria()
        def results = c {
            projections {
                max('loteOro')
            }}
        def maxLote = results.get(0)?: 0
        this.loteOro = maxLote + 1

        this.codigoDepositoOro = RecepcionDeComplejo.countByDeposito(this.deposito) + 1
    ...
    String toString(){
        ...
        def s = ""
        if (loteOro)
            return "${deposito?.codigoDeposito}-${decimalFormat.format(loteOro)}"
