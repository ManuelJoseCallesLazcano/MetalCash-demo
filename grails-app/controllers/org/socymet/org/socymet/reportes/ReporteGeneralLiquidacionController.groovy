...
@Secured(['ROLE_ADMIN'])
class ReporteGeneralLiquidacionController {

    static allowedMethods = [save: "POST", update: "PUT", delete: "DELETE"]

    def index(Integer max) {
        params.max = Math.min(max ?: 10, 100)
        respond ReporteGeneralLiquidacion.list(params), model: [reporteGeneralLiquidacionInstanceCount: ReporteGeneralLiquidacion.count()]
    }

    def show(ReporteGeneralLiquidacion reporteGeneralLiquidacionInstance) {
        respond reporteGeneralLiquidacionInstance
    }

    def create() {
        respond new ReporteGeneralLiquidacion(params)
    }

    @Transactional
    def save(ReporteGeneralLiquidacion reporteGeneralLiquidacionInstance) {
        if (reporteGeneralLiquidacionInstance == null) {
            notFound()
            return
        }

        if (reporteGeneralLiquidacionInstance.hasErrors()) {
            respond reporteGeneralLiquidacionInstance.errors, view: 'create'
            return
        }

        reporteGeneralLiquidacionInstance.save flush: true

        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.created.message', args: [message(code: 'reporteGeneralLiquidacion.label', default: 'ReporteGeneralLiquidacion'), reporteGeneralLiquidacionInstance.id])
                redirect reporteGeneralLiquidacionInstance
            }
            '*' { respond reporteGeneralLiquidacionInstance, [status: CREATED] }
        }
    }

    def edit(ReporteGeneralLiquidacion reporteGeneralLiquidacionInstance) {
        respond reporteGeneralLiquidacionInstance
    }

    @Transactional
    def update(ReporteGeneralLiquidacion reporteGeneralLiquidacionInstance) {
        if (reporteGeneralLiquidacionInstance == null) {
            notFound()
            return
        }
        ...
        if (reporteGeneralLiquidacionInstance.hasErrors()) {
            respond reporteGeneralLiquidacionInstance.errors, view: 'edit'
            return
        }
        ...
        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.updated.message', args: [message(code: 'ReporteGeneralLiquidacion.label', default: 'ReporteGeneralLiquidacion'), reporteGeneralLiquidacionInstance.id])
                redirect reporteGeneralLiquidacionInstance
            }
            '*' { respond reporteGeneralLiquidacionInstance, [status: OK] }
        }
    }

    @Transactional
    def delete(ReporteGeneralLiquidacion reporteGeneralLiquidacionInstance) {
        ...
        reporteGeneralLiquidacionInstance.delete flush: true

        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.deleted.message', args: [message(code: 'ReporteGeneralLiquidacion.label', default: 'ReporteGeneralLiquidacion'), reporteGeneralLiquidacionInstance.id])
    ...
    protected void notFound() {
        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.not.found.message', args: [message(code: 'reporteGeneralLiquidacion.label', default: 'ReporteGeneralLiquidacion'), params.id])
                redirect action: "index", method: "GET"
            }
...
//        def tipoReporte = "fechasEmpresa"+params.tipoReporte
        ...
        log.error("params.deposito: ${params.deposito}")
        log.error("params.deposito.id: ${params.deposito.id}")
        def deposito = params.deposito.id.toString().isNumber()?Deposito.get(params.deposito.id.toLong()):null
        ...
        if (tipoReporte.equals("fechasEmpresa")){
            ...
            if(deposito)
                liquidacionesComplejo = LiquidacionDeComplejo.findAllByDepositoAndFechaDeLiquidacionBetween(deposito, fechaInicial,fechaFinal,[sort: 'fechaDeLiquidacion'])
            else
                liquidacionesComplejo = LiquidacionDeComplejo.findAllByFechaDeLiquidacionBetween(fechaInicial,fechaFinal,[sort: 'fechaDeLiquidacion'])
