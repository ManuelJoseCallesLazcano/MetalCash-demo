...
import org.socymet.liquidacion.LiquidacionDeAntimonioRetenciones
...
import org.socymet.liquidacion.LiquidacionDeWolfranRetenciones
...
class DetalleComprasController {
    ...
    def index(Integer max) {
        ...
        respond DetalleCompras.list(params), model:[detalleComprasInstanceCount: DetalleCompras.count()]
    }

    def show(DetalleCompras detalleComprasInstance) {
        respond detalleComprasInstance
    }

    def create() {
        respond new DetalleCompras(params)
    }

    @Transactional
    def save(DetalleCompras detalleComprasInstance) {
        if (detalleComprasInstance == null) {
            notFound()
            return
        }

        if (detalleComprasInstance.hasErrors()) {
            respond detalleComprasInstance.errors, view:'create'
            return
        }

        detalleComprasInstance.save flush:true

        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.created.message', args: [message(code: 'detalleCompras.label', default: 'DetalleCompras'), detalleComprasInstance.id])
                redirect detalleComprasInstance
            }
            '*' { respond detalleComprasInstance, [status: CREATED] }
        }
    }

    def edit(DetalleCompras detalleComprasInstance) {
        respond detalleComprasInstance
    }

    @Transactional
    def update(DetalleCompras detalleComprasInstance) {
        if (detalleComprasInstance == null) {
            notFound()
            return
        }
        ...
        if (detalleComprasInstance.hasErrors()) {
            respond detalleComprasInstance.errors, view:'edit'
            return
        }
        ...
        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.updated.message', args: [message(code: 'DetalleCompras.label', default: 'DetalleCompras'), detalleComprasInstance.id])
                redirect detalleComprasInstance
            }
            '*'{ respond detalleComprasInstance, [status: OK] }
        }
    }

    @Transactional
    def delete(DetalleCompras detalleComprasInstance) {
        ...
        detalleComprasInstance.delete flush:true

        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.deleted.message', args: [message(code: 'DetalleCompras.label', default: 'DetalleCompras'), detalleComprasInstance.id])
    ...
    protected void notFound() {
        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.not.found.message', args: [message(code: 'detalleCompras.label', default: 'DetalleCompras'), params.id])
                redirect action: "index", method: "GET"
            }
    ...
    def crearReporte = {
        ...
        WritableSheet sheet1 = workbook.createSheet("Detalle de compra", 0)
        ...
        response.setHeader('Content-Disposition', 'Attachment;Filename="detalle_compras_'+empresa.nombreDeEmpresa.toLowerCase().replace(' ','_')+'.xls"')
...
//        }
        ...
        sheet1.addCell(new Label(0,1, "DETALLE DE COMPRAS ${empresa.toString()}",formatoTitulo))
...
//        sheet1.addCell(new Label(9,6, "NOMBRE",formatoEncabezado))
        sheet1.addCell(new Label(9,6, "LOTE",formatoEncabezado))
        sheet1.addCell(new Label(10,6, "ELEMENTO",formatoEncabezado))
//        sheet1.addCell(new Label(11,6, "LOTE",formatoEncabezado))
        sheet1.addCell(new Label(11,6, "NOMBRES Y APELLIDOS",formatoEncabezado))
...
//                sheet1.addCell(new Label(9,fila, it.nombreCliente,formatoDatos))
                sheet1.addCell(new Label(9,fila, it.lote,formatoDatos))
...
//                sheet1.addCell(new Number(44,fila, it.recepcionDeComplejo.alicuota.plata,formatoDatos)) //AG
        ...
        //tipo de cambio com celda vacia
        sheet1.addCell(new Label(44,f, "",formatoTotales))
...
//        def columnasOcultas = [columnaFinalRetenciones-1,columnaFinalRetenciones-2,columnaFinalRetenciones-6,columnaFinalRetenciones-8,columnaFinalRetenciones-retencionesOtras.size()-9,49,48,47,45,43,42,41,40,39,38,37,36,35,34,33,32,31,27,26,25,24,20,19,18,14,12,10,8,7,6,5,4,3,2,1]
        def columnasOcultas = [columnaFinalRetenciones-1,columnaFinalRetenciones-2,columnaFinalRetenciones-3,columnaFinalRetenciones-4,columnaFinalRetenciones-5,columnaFinalRetenciones-6,columnaFinalRetenciones-8,columnaFinalRetenciones-8,columnaFinalRetenciones-retencionesOtras.size()-9,49,48,47,45,43,42,41,40,39,38,37,36,35,34,33,32,31,27,26,25,24,20,19,18,14,12,10,8,7,6,5,4,3,2,1]
