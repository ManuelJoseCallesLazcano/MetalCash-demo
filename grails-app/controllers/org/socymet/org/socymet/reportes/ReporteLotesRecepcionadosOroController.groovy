...
class ReporteLotesRecepcionadosOroController {

    static allowedMethods = [save: "POST", update: "PUT", delete: "DELETE"]

    def index(Integer max) {
        params.max = Math.min(max ?: 10, 100)
        respond ReporteLotesRecepcionadosOro.list(params), model:[reporteLotesRecepcionadosOroInstanceCount: ReporteLotesRecepcionadosOro.count()]
    }

    def show(ReporteLotesRecepcionadosOro reporteLotesRecepcionadosOroInstance) {
        respond reporteLotesRecepcionadosOroInstance
    }

    def create() {
        respond new ReporteLotesRecepcionadosOro(params)
    }

    @Transactional
    def save(ReporteLotesRecepcionadosOro reporteLotesRecepcionadosOroInstance) {
        if (reporteLotesRecepcionadosOroInstance == null) {
            notFound()
            return
        }

        if (reporteLotesRecepcionadosOroInstance.hasErrors()) {
            respond reporteLotesRecepcionadosOroInstance.errors, view:'create'
            return
        }

        reporteLotesRecepcionadosOroInstance.save flush:true

        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.created.message', args: [message(code: 'reporteLotesRecepcionadosOro.label', default: 'ReporteLotesRecepcionadosOro'), reporteLotesRecepcionadosOroInstance.id])
                redirect reporteLotesRecepcionadosOroInstance
            }
            '*' { respond reporteLotesRecepcionadosOroInstance, [status: CREATED] }
        }
    }

    def edit(ReporteLotesRecepcionadosOro reporteLotesRecepcionadosOroInstance) {
        respond reporteLotesRecepcionadosOroInstance
    }

    @Transactional
    def update(ReporteLotesRecepcionadosOro reporteLotesRecepcionadosOroInstance) {
        if (reporteLotesRecepcionadosOroInstance == null) {
            notFound()
            return
        }
        ...
        if (reporteLotesRecepcionadosOroInstance.hasErrors()) {
            respond reporteLotesRecepcionadosOroInstance.errors, view:'edit'
            return
        }
        ...
        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.updated.message', args: [message(code: 'ReporteLotesRecepcionadosOro.label', default: 'ReporteLotesRecepcionadosOro'), reporteLotesRecepcionadosOroInstance.id])
                redirect reporteLotesRecepcionadosOroInstance
            }
            '*'{ respond reporteLotesRecepcionadosOroInstance, [status: OK] }
        }
    }

    @Transactional
    def delete(ReporteLotesRecepcionadosOro reporteLotesRecepcionadosOroInstance) {
        ...
        reporteLotesRecepcionadosOroInstance.delete flush:true

        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.deleted.message', args: [message(code: 'ReporteLotesRecepcionadosOro.label', default: 'ReporteLotesRecepcionadosOro'), reporteLotesRecepcionadosOroInstance.id])
    ...
    protected void notFound() {
        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.not.found.message', args: [message(code: 'reporteLotesRecepcionadosOro.label', default: 'ReporteLotesRecepcionadosOro'), params.id])
                redirect action: "index", method: "GET"
            }
