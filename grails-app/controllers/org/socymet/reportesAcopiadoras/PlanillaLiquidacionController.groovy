...
class PlanillaLiquidacionController {
    ...
    def index(Integer max) {
        ...
        respond PlanillaLiquidacion.list(params), model: [planillaLiquidacionInstanceCount: PlanillaLiquidacion.count()]
    }

    def show(PlanillaLiquidacion planillaLiquidacionInstance) {
        respond planillaLiquidacionInstance
    }

    def create() {
        respond new PlanillaLiquidacion(params)
    }

    @Transactional
    def save(PlanillaLiquidacion planillaLiquidacionInstance) {
        if (planillaLiquidacionInstance == null) {
            notFound()
            return
        }

        if (planillaLiquidacionInstance.hasErrors()) {
            respond planillaLiquidacionInstance.errors, view: 'create'
            return
        }

        planillaLiquidacionInstance.save flush: true

        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.created.message', args: [message(code: 'planillaLiquidacion.label', default: 'PlanillaLiquidacion'), planillaLiquidacionInstance.id])
                redirect planillaLiquidacionInstance
            }
            '*' { respond planillaLiquidacionInstance, [status: CREATED] }
        }
    }

    def edit(PlanillaLiquidacion planillaLiquidacionInstance) {
        respond planillaLiquidacionInstance
    }

    @Transactional
    def update(PlanillaLiquidacion planillaLiquidacionInstance) {
        if (planillaLiquidacionInstance == null) {
            notFound()
            return
        }
        ...
        if (planillaLiquidacionInstance.hasErrors()) {
            respond planillaLiquidacionInstance.errors, view: 'edit'
            return
        }
        ...
        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.updated.message', args: [message(code: 'PlanillaLiquidacion.label', default: 'PlanillaLiquidacion'), planillaLiquidacionInstance.id])
                redirect planillaLiquidacionInstance
            }
            '*' { respond planillaLiquidacionInstance, [status: OK] }
        }
    }

    @Transactional
    def delete(PlanillaLiquidacion planillaLiquidacionInstance) {
        ...
        planillaLiquidacionInstance.delete flush: true

        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.deleted.message', args: [message(code: 'PlanillaLiquidacion.label', default: 'PlanillaLiquidacion'), planillaLiquidacionInstance.id])
    ...
    protected void notFound() {
        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.not.found.message', args: [message(code: 'planillaLiquidacion.label', default: 'PlanillaLiquidacion'), params.id])
                redirect action: "index", method: "GET"
            }
...
//        def tipoReporte = "fechasEmpresa"+params.tipoReporte
        ...
        if (tipoReporte.equals("fechasEmpresa")){
            ...
            def liquidacionesCm = LiquidacionDeComplejo.findAllByDeposito(usuarioActual.deposito,[sort: 'lote'])
