...
<head>
    ...
    <title>Editar Pago de Transporte</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" type="text/css">
    <style>
        .select2-container--default .select2-selection--single {
            height: calc(1.5em + .75rem + 2px); padding: .375rem .75rem; border: 1px solid #ced4da; border-radius: .25rem;
        }
...
<div class="card card-primary">
    <div class="card-header">
        <h3 class="card-title">Editar Pago de Transporte
            <span class="badge badge-light ml-1">${pagoTransporteInstance}</span>
        </h3>
    </div>
    <g:form method="post" name="pagoForm">
        <g:hiddenField name="vista" value="edit"/>
        <g:hiddenField name="id" value="${pagoTransporteInstance?.id}"/>
        <g:hiddenField name="version" value="${pagoTransporteInstance?.version}"/>
        <div class="card-body">
        ...
        <div class="card-footer">
            ...
            <g:link action="show" id="${pagoTransporteInstance?.id}" class="btn btn-secondary ml-1">Cancelar</g:link>
        </div>
    </g:form>
</div>
</body>
