$(document).ready(function() {
    ...
    if($('#empresa\\.id').val()!=""){ // vista edit
        var s = $('#tablaCobre').val();
        var t = $('#terminosDeContrato').val();
        $.ajax({
            url:"/demo-liquidaciones/recepcionDeComplejo/preciosCobreIds",
            dataType: 'json',
            data: {
    ...
    //$("#porcentajeCobre, #porcentajePlata, #porcentajeZinc, #dolarPuntoCobre, #dolarPuntoPlata, #dolarPuntoZinc, #porcentajeRegalia, #totalAnticiposContraFuturaEntrega").bind("keyup",calcularTotalLiquidoPagable.bind("keyup",generarTablaRetencionesComplejo));
    ...
    $("#lote").autocomplete({
        source: function(request, response){
            $.ajax({
                url: "/demo-liquidaciones/recepcionDeComplejo/recepcionesCobrePlataJSON", // /demo-liquidaciones/recepcionDeComplejo/create
                data: request,
                success: function(data){
        ...
        select: function(event, ui) { // event handler when user selects a company from the list.
            ...
            //kilosNetosHumedos
            $('#cotizacionDiariaDeCobre').val(ui.item.cotizacionDiariaDeCobre);
            $('#cotizacionQuincenalDeCobre').val(ui.item.cotizacionQuincenalDeCobre);
            $('#alicuotaDeCobre').val(ui.item.alicuotaDeCobre);
            ...
            $('#porcentajeCobrePromexbol').val(ui.item.porcentajeCobrePromexbol);
            ...
            if(ui.item.controlCalidadId!=0){
                ...
                $('<a>',{
                    ...
                    href: '/demo-liquidaciones/controlCalidadCobrePlata/show/'+ui.item.controlCalidadId,
                    target: '_blank'
            ...
            if(ui.item.merma=="1"&&ui.item.humedad=="1"&&ui.item.porcentajeZinc=="1"&&ui.item.porcentajeCobre=="1"&&ui.item.porcentajePlata=="1"){
            ...
            $('#porcentajeCobreCliente').prop("readonly", false);
    ...
    $('#valorar').click(function() {
        $.ajax({
            ...
            url:"/demo-liquidaciones/recepcionDeComplejo/recepcionesCobrePlataValoracionJSON",
            dataType: 'json',
            data: {
                vista: $('#vista').val(),
            ...
            success: function(data) {
                ...
                //kilosNetosHumedos
                $('#cotizacionDiariaDeCobre').val(data.cotizacionDiariaDeCobre);
                $('#cotizacionQuincenalDeCobre').val(data.cotizacionQuincenalDeCobre);
                $('#alicuotaDeCobre').val(data.alicuotaDeCobre);
                ...
                $('#kilosFinosCobre').val(data.kilosFinosCobre);
                $('#kilosFinosPlata').val(data.kilosFinosPlata);
                $('#librasFinasDeCobre').val(data.librasFinasDeCobre);
                $('#onzasTroyDePlata').val(data.onzasTroyDePlata);
                $('#valorOficialBrutoDeCobre').val(data.valorOficialBrutoDeCobre);
                $('#valorOficialBrutoDePlata').val(data.valorOficialBrutoDePlata);
                $('#valorOficialBrutoDeCobreEnBolivianos').val(data.valorOficialBrutoDeCobreEnBolivianos);
                ...
                if(data.merma=="1"&&data.humedad=="1"&&data.porcentajeZinc=="1"&&data.porcentajeCobre=="1"&&data.porcentajePlata=="1"){
                    $.jGrowl("Aun no se han registrado las LEYES para el lote ingresado.", {sticky: true, header: 'ATENCION'});
                    $("#create").prop("disabled",true);
                }else
                    $("#create").prop("disabled",false);
    ...
    $('#copiarLeyes').click(function() {
        $("#porcentajeCobreFinal, #porcentajeCobreCliente, #porcentajeCobre").val($("#porcentajeCobrePromexbol").val());
    ...
    $("#porcentajeCobreCliente").keyup(function(){
        var leyPromexbol = parseFloat($("#porcentajeCobrePromexbol").val());
        var leyCliente = parseFloat($("#porcentajeCobreCliente").val());
        var diferencia = leyCliente-leyPromexbol
        if(diferencia>3){//la ley del cliente no puede ser mayor en 3 puntos a la de promexbol
            $.jGrowl("La ley del cliente no puede ser mayor en 3 puntos a la de EMPRESA.", { sticky: true });
            $("#porcentajeCobreFinal, #porcentajeCobre").val(0);
        }else{
            var leyFinal = (leyPromexbol+leyCliente)/2;
            $("#porcentajeCobreFinal, #porcentajeCobre").val(leyFinal);
        }
    });
    ...
    function simplificarOpciones(tablasIds, terminosIds){
        ...
        $('#tablaCobre option[value]').each(function() {
    ...
    function respaldarOpciones(){
        var pos=0;
        $('#tablaCobre option').each(function() {
    ...
    function copiarOpciones(){
        ...
        for(var i=0;i<opcionesValue.length;i++){
            $("#tablaCobre").append("<option value=\""+opcionesValue[i]+"\">"+opcionesText[i]+"</option>");
        }
    ...
    function actualizarLeyes(){
        $.ajax({
            url:"/demo-liquidaciones/controlCalidadCobrePlata/leyes",
            dataType: 'json',
            data: {
                recepcionDeComplejoId: $("#recepcionDeComplejo\\.id").val()
            },
            success: function(data) {
                ...
                var cobreAnterior = transFloat($("#porcentajeCobrePromexbol").val());
                ...
                var cobreNueva = data.porcentajeCobrePromexbol;
                var plataNueva = data.porcentajePlataPromexbol;

                if(mermaNueva==0 && humedadNueva==0 && cobreNueva==0 && plataNueva==0) return;

                if(mermaAnterior!=mermaNueva || humedadAnterior!=humedadNueva || cobreAnterior!=cobreNueva || plataAnterior!=plataNueva){
                    $('#porcentajeMermaPromexbol').val(data.porcentajeMermaPromexbol);
                    $('#porcentajeHumedadPromexbol').val(data.porcentajeHumedadPromexbol);
                    $('#porcentajeCobrePromexbol').val(data.porcentajeCobrePromexbol);
