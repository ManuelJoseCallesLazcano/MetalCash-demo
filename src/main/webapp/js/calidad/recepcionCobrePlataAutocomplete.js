$(document).ready(function() {
    ...
    $("#lote").autocomplete({
        source: function(request, response){
            $.ajax({
                url: "/demo-liquidaciones/recepcionDeComplejo/recepcionesCalidadCobrePlataJSON", // /demo-liquidaciones/recepcionDeComplejo/create
                data: request,
    ...
    $("#porcentajeCobrePromexbol").keyup(function(){
        $("#porcentajeCobreCliente,#porcentajeCobreFinal").val($("#porcentajeCobrePromexbol").val());
    });
    ...
    $("#porcentajeCobreCliente").keyup(function(){
        ...
        if(diferencia>3){//la ley del cliente no puede ser mayor en 3 puntos a la de promexbol
            $.jGrowl("La ley del cliente no puede ser mayor en 3 puntos a la de EMPRESA.", { sticky: true });
            $("#porcentajeCobreFinal").val(0);
        }else{
            var leyFinal = (leyPromexbol+leyCliente)/2;
            $("#porcentajeCobreFinal").val(leyFinal);
        }
    });
