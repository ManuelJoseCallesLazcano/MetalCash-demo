$(document).ready(function() {
    ...
    $("#lote").autocomplete({
        source: function(request, response){
            $.ajax({
                url: "/demo-liquidaciones/recepcionDeComplejo/recepcionesZincPlataJSON", // /demo-liquidaciones/recepcionDeComplejo/create
                data: request,
                success: function(data){
        ...
        select: function(event, ui) { // event handler when user selects a company from the list.
            ...
            if(ui.item.controlCalidadId!=0){
                ...
                $('<a>',{
                    ...
                    href: '/demo-liquidaciones/controlCalidadZincPlata/show/'+ui.item.controlCalidadId,
                    target: '_blank'
    ...
    $('#valorar').click(function() {
        $.ajax({
            ...
            url:"/demo-liquidaciones/recepcionDeComplejo/recepcionesZincPlataValoracionJSON",
            dataType: 'json',
            data: {
                vista: $('#vista').val(),
            ...
            success: function(data) {
                ...
                if(data.merma=="1"&&data.humedad=="1"&&data.porcentajeZinc=="1"&&data.porcentajeZinc=="1"&&data.porcentajePlata=="1"){
                    $.jGrowl("Aun no se han registrado las LEYES para el lote ingresado.", {sticky: true, header: 'ATENCION'});
                    $("#create").prop("disabled",true);
                }else
                    $("#create").prop("disabled",false);
    ...
    $('#copiarLeyes').click(function() {
        $("#porcentajeZincFinal, #porcentajeZincCliente, #porcentajeZinc").val($("#porcentajeZincPromexbol").val());
    ...
    $("#porcentajeZincCliente").keyup(function(){
        ...
        if(diferencia>3){//la ley del cliente no puede ser mayor en 3 puntos a la de promexbol
            $.jGrowl("La ley del cliente no puede ser mayor en 3 puntos a la de EMPRESA.", { sticky: true });
            $("#porcentajeZincFinal, #porcentajeZinc").val(0);
        }else{
            var leyFinal = (leyPromexbol+leyCliente)/2;
            $("#porcentajeZincFinal, #porcentajeZinc").val(leyFinal);
        }
    });
    ...
    function actualizarLeyes(){
        $.ajax({
            url:"/demo-liquidaciones/controlCalidadZincPlata/leyes",
            dataType: 'json',
            data: {
                recepcionDeComplejoId: $("#recepcionDeComplejo\\.id").val()
            },
            success: function(data) {
                ...
                if(mermaNueva==0 && humedadNueva==0 && zincNueva==0 && plataNueva==0) return;

                if(mermaAnterior!=mermaNueva || humedadAnterior!=humedadNueva || zincAnterior!=zincNueva || plataAnterior!=plataNueva){
