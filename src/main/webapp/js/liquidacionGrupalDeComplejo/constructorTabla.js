...
$(document).ready(function() {
    ...
    function buscarFilas(){
        $.ajax({
            url: "/demo-liquidaciones/recepcionDeComplejo/recepcionGrupalComplejoJSON",
            dataType: 'json',
            data: {
    ...
    function insertarFilas(){
        ...
        for(var i=0;i<data.length;i++){
            ...
            $("#nuevaTabla").find('tbody')
                ...
                .append($('<td>')
                    .append($('<input>')
                        .attr('id', 'porcentajeZincCliente_'+i)
                        .attr('size', '5')
                        .val(data[i].porcentajeZincCliente)
                        .bind("keyup",function(){
                            promediarLey("Zinc",this);
                        })
                    )
                )
                .append($('<td>')
                ...
                .append($('<td>')
                    .append($('<input>')
                        .attr('id', 'porcentajeZincFinal_'+i)
                        .attr('size', '5')
                        .attr('readonly', 'true')
                        .val(data[i].porcentajeZincFinal)
                    )
                )
                .append($('<td>')
                    .append($('<input>')
    ...
    function promediarLey(elemento,entrada){
        ...
        if(elemento=="Zinc"){
            porcentajePromexbol = transFloat(lotesJSON[i].porcentajeZincPromexbol);
    ...
    function copiarLeyes(){
        for(var i=0;i<lotesJSON.length;i++){
            $("#porcentajeZincCliente_"+i).val(lotesJSON[i].porcentajeZincPromexbol);
            $("#porcentajeZincFinal_"+i).val(lotesJSON[i].porcentajeZincPromexbol);
            lotesJSON[i].porcentajeZincCliente=lotesJSON[i].porcentajeZincPromexbol;
            lotesJSON[i].porcentajeZincFinal=lotesJSON[i].porcentajeZincPromexbol;
    ...
    function valorar(boton){
        ...
        lotesJSON[i].porcentajeZincCliente=$("#porcentajeZincCliente_"+i).val();
        lotesJSON[i].porcentajeZincFinal=$("#porcentajeZincFinal_"+i).val();
        ...
        cotizacionQuincenalZinc=lotesJSON[i].cotizacionQuincenalDeZinc;
        ...
        alicuotaZinc=lotesJSON[i].alicuotaDeZinc;
        ...
        porcentajeZinc = lotesJSON[i].porcentajeZincFinal;
        ...
        valorOficialBruto = valorOficialBrutoZincBs+valorOficialBrutoPlomoBs+valorOficialBrutoPlataBs;

        //determinacion del valor por tonelada
        if(lotesJSON[i].modoValoracion=="TABLA"){
            valorToneladaZinc = valorToneladaZincPorTabla(lotesJSON[i].recepcionId,lotesJSON[i].tablaComplejoId,lotesJSON[i].porcentajeZincFinal);
        ...
        if(lotesJSON[i].modoValoracion=="TERMINOS DE CONTRATO"){
            valorToneladaTerminos = valorToneladaPorTerminos(lotesJSON[i].recepcionId,lotesJSON[i].terminosDeContratoId,lotesJSON[i].porcentajeZincFinal,lotesJSON[i].porcentajePlomoFinal,lotesJSON[i].porcentajePlataFinal,0);
            valorTonelada = valorToneladaTerminos+margen;
            //alert("TERMINOS: margen i="+i+" -> "+margen+" -> vpt="+valorToneladaTerminos);
        }

        regaliaMinera = (valorOficialBrutoZincBs*alicuotaZinc/100 + valorOficialBrutoPlomoBs*alicuotaPlomo/100 + valorOficialBrutoPlataBs*alicuotaPlata/100);
        ...
        lotesJSON[i].valorOficialBrutoZinc=valorOficialBrutoZinc;
        lotesJSON[i].valorOficialBrutoPlomo=valorOficialBrutoPlomo;
        lotesJSON[i].valorOficialBrutoPlata=valorOficialBrutoPlata;
        lotesJSON[i].valorOficialBrutoZincBs=valorOficialBrutoZincBs;
        ...
        lotesJSON[i].valorToneladaZinc=valorToneladaZinc;
    ...
    function liquidar(boton){
        ...
        if(confirm("Está seguro que desea liquidar el Lote "+lotesJSON[i].lote+"?")){
            ...
            $.ajax({
                url: "/demo-liquidaciones/liquidacionDeComplejo/liquidar",
                async: false,
                dataType: 'json',
                data: {
...
//        });
        ...
        $('<form>', {
            ...
            "action": '/demo-liquidaciones/liquidacionDeComplejo/crearReporteGrupal2'
    ...
    function mostrarTablaLotesLiquidados(){
        $("#tablaLotesLiquidados").jqGrid({
            datatype: "local",
            height: 150,
            colNames: ["LOTE","%ZN FINAL","%PB FINAL","DM AG FINAL","MODO VAL.","MARGEN","V.P.T.","ANTICIPOS C/E","LIQ. PAGABLE"],
            colModel:[
                {name:'lote',index:'lote', width:80},
                {name:'porcentajeZincFinal',index:'porcentajeZincFinal', width:80},
    ...
    function valorToneladaZincPorTabla(recepcionId,tablaComplejoId,porcentajeZinc){
        var vpt=0;
        $.ajax({
            url: "/demo-liquidaciones/tablaOrigenCotizacionesComplejo/getVPTZinc2",
            async: false,
            dataType: 'json',
            data: {
                recepcionId: recepcionId,
